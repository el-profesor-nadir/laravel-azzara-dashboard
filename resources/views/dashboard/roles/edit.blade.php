...
<div class="page-header">
    <h4 class="page-title">@lang('Edit role') : {{ $role->name }}</h4>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <form class="forms-sample" method="POST" action="{{ route('dashboard.roles.update', $role) }}">
                    @csrf
                    {{ method_field('PUT') }}
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input id="name" type="text" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name"
                                value="{{ old('name', optional($role)->name) }}" placeholder="Name" required autofocus>
                            @if ($errors->has('name'))
                                ...
                                @foreach ($permissions as $permission)
                                    ...
                                    <div class="custom-control custom-checkbox d-block">
                                        <input class="custom-control-input {{ $errors->has('permissions') ? ' is-invalid' : '' }}" type="checkbox" value="{{ $permission->id }}"
                                            name="permissions[]" id="{{ $permission->name }}" {{ in_array($permission->id, old('permissions', [])) ? 'checked' : ($role->permissions->contains($permission) ? 'checked' : '') }}>
